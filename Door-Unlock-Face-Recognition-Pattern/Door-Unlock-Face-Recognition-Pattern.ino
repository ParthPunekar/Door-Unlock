...
#define SWITCH2 13
#define SWITCH3 15
#define PATTERN_LED 14

unsigned long door_opened_millis = 0;
unsigned long pattern_millis = 0;
...
void setup() {
  ...
  pinMode(PATTERN_LED, OUTPUT);

  digitalWrite(relay_pin, LOW);
  digitalWrite(PATTERN_LED, LOW);
...
void patternUnlock(WebsocketsClient &client) {

  while (millis() - pattern_millis < 4000) {
    if (count2 == 0 && count3 == 0 && digitalRead(SWITCH1) == LOW) {
      count1 += 1;
      delay(300);
    } else if (count1 == 2 && count3 == 1 && digitalRead(SWITCH2) == LOW) {
      count2 += 1;
      delay(300);
    } else if (count1 == 2 && count2 == 0 && digitalRead(SWITCH3) == LOW) {
      count3 += 1;
      delay(300);
    }
  }
        ...
        if (align_face(out_res.net_boxes, image_matrix, aligned_face) == ESP_OK)
        {
          ...
          if (g_state == START_RECOGNITION  && (st_face_list.count > 0))
          {
            ...
            else
            {
              ...
              delay(1000);
              client.send("Enter Pattern to UNLOCK");
              digitalWrite(PATTERN_LED, HIGH);
              count1 = 0;
              count2 = 0;
              count3 = 0;
              pattern_millis = millis();
              patternUnlock(client);
            }
          }
